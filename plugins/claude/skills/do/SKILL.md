...
5. **コーディング作業は Codex MCP に委譲** - 詳細設計と行動計画の作成までを担い、実装・テストは Codex に委譲する。Codex が利用不可の場合は subagent が直接実装する。プロジェクトルール (`.claude/rules/`) を Codex に渡して準拠させる
...
- **各タスクがコーディングを伴うか判定** (ファイル作成・変更・テスト作成 → コーディングタスク)
