...
| 絶対上限       | 60 分 (`HAD_ACTIVITY` に関わらず強制終了)                  |
...
| `TIMEOUT_ABS\|Xmin`                    | 絶対タイムアウト (60 分)           | 監視終了 → 完了報告 (4)     |
...
- `HANDLED_THREADS`: 対応済みレビュースレッド ID のリスト (同じスレッドの再処理をスキップする)
...
while true; do
  ...
  if [ $? -ne 0 ]; then
    ...
    [ $API_ERRORS -ge 3 ] && echo "TIMEOUT_ABS|${ELAPSED}min|api_error" && exit 1
    sleep 60; continue
  fi
  API_ERRORS=0
  ...
  [ "$MG" = "CONFLICTING" ] && echo "PR_CONFLICT"
  ...
  # CI ステータスチェック
  if [ -n "$SHA" ]; then
    RJ=$(gh run list --commit "$SHA" -R "$OWNER/$REPO" --json databaseId,status,conclusion,name -L 50 2>/dev/null) || { sleep 60; continue; }
...
通知に含まれるスレッド ID のうち、`HANDLED_THREADS` に含まれないものを処理対象とする。
...
8. 処理したスレッド ID を `HANDLED_THREADS` に追加する
