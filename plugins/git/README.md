...
```bash
/git:pr-watch      # 現在のブランチの PR を監視
/git:pr-watch 123  # PR #123 を監視
```

**ワークフロー:**

1. PR のレビューコメントと CI 失敗を **2 分間隔で定期監視**
2. 未解決のレビューコメントを検出 → 妥当性判断 → **自動修正・コミット・プッシュ**
3. CI 失敗を検出 → **ci-analyzer subagent** で原因分析 → **自動修正・コミット・プッシュ**
4. レビューコメントに **自動返信・resolve・+1 リアクション**
