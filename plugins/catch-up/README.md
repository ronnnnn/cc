...
技術・ツール・フレームワークの最新バージョン取得・技術調査・セッション内容の学習を支援する Claude Code plugin です。
...
| `learn`         | セッション内容を対話とクイズで深く理解させる学習ガイド |
...
セッションで行った作業を深く理解したいときに `learn` スキルが起動します:
...
理解度チェックリストを `.claude/learn/learn-<トピックの slug>.md` として永続化し、問題・解決策・広い文脈の 3 階層を対話とクイズ (`AskUserQuestion`) で確認しながら、全項目を習得するまで段階的に学習を進めます。
...
│       ├── SKILL.md             # セッション学習スキル
│       └── references/
│           ├── checklist-template.md  # 理解度チェックリストのテンプレート
│           └── teaching-guide.md      # 説明レベル・クイズ・デバッガ活用の詳細
└── README.md
```
