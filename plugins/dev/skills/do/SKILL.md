...
5. **コーディング作業の責務分担**
   - (1) subagent の責務は詳細設計と行動計画の作成までとする
   - (2) 実装・テストは、対象コードの規模や複雑性に関わらず、1 行の変更であっても必ず Codex MCP に委譲する
   - (3) Codex MCP が利用不可の場合に限り、subagent が直接実装・テストを行う
   - (4) プロジェクトルール (`.claude/rules/`) を Codex MCP に渡し、その内容を遵守させる
