...
5. **コーディング作業は必ず Codex MCP に委譲** - 対象コードの規模や複雑性に関わらず、1 行の変更であっても全てのコーディング作業を Codex に委譲する。詳細設計と行動計画の作成までを担い、実装・テストは Codex に委譲する。Codex が利用不可の場合のみ subagent が直接実装する。プロジェクトルール (`.claude/rules/`) を Codex に渡して準拠させる
...
TaskCreate({ subject: "詳細設計の作成", description: "コーディングタスクの設計書を作成", activeForm: "詳細設計を作成中" })
...
- **各タスクがコーディングを伴うか判定** (ファイル作成・変更・テスト作成・設定ファイル編集・1 行の修正等、コード変更を伴うものは全てコーディングタスクとして Codex に委譲)
...
### 2. 詳細設計の作成 (コーディングタスクのみ)

コーディングタスクごとに、関連コードを Read/Glob/Grep で調査し、ファイル単位の変更設計書を作成する:
...
1. **path/to/file1** - <変更内容の概要>
2. **path/to/file2** - <変更内容の概要>

### 新規作成ファイル

1. **path/to/new-file** - <目的と概要>
...
この設計書はガイダンスであり、Codex がより良いアプローチを判断した場合は設計変更を許容する。

### 3. アプローチ判定

#### 3-1. Agent Teams の利用可能性確認
...
ステップ 2 で作成した設計書と `references/codex-delegation.md` の実行手順を組み合わせて subagent のプロンプトを構築する。

```
Task({
  subagent_type: "general-purpose",
  description: "<タスクの要約>",
  prompt: `以下のコーディングタスクを Codex MCP に委譲して実行する。
...
- `run_in_background: true` を指定した subagent は MCP サーバーのツールを利用できない (バックグラウンドプロセスの制約)。Codex 委譲タスクにはこのオプションを指定しないこと
