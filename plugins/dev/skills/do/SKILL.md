---
name: do
description: ユーザーが列挙した複数の独立タスクを並列で実行し、結果を集約して報告する。タスク内容に応じて subagent または Agent Teams を自動選択する。Use when 複数の独立したタスクを「並列で」「同時に」「まとめて」「一気に」進めるよう依頼された、互いに独立したタスクが番号や箇条書きで列挙された、swarm 実行を求められた際に使用する。1 つの実装依頼に付随する受け入れ条件やチェックリストのように項目が依存し合う場合は使わない。自分で Agent を起動したりファイル調査を始めたりする前に、必ずこのスキルを最初に呼び出す。
